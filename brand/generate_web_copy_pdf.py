"""Generate a PDF of the web copy V1 draft for Saskia to review."""

from __future__ import annotations

from pathlib import Path

from reportlab.lib import colors
from reportlab.lib.colors import HexColor
from reportlab.lib.enums import TA_LEFT
from reportlab.lib.pagesizes import A4
from reportlab.lib.styles import ParagraphStyle, getSampleStyleSheet
from reportlab.lib.units import mm
from reportlab.pdfbase import pdfmetrics
from reportlab.pdfbase.ttfonts import TTFont
from reportlab.platypus import (
    BaseDocTemplate,
    Frame,
    PageTemplate,
    PageBreak,
    NextPageTemplate,
    Paragraph,
    Spacer,
    HRFlowable,
    KeepTogether,
    ListFlowable,
    ListItem,
)


OUTPUT = Path(__file__).parent.parent / "content" / "web_copy_v1.pdf"

NAVY = HexColor("#0E1A3A")
GOLD = HexColor("#B8893A")
VIOLET = HexColor("#6B4E8E")
MUTED = HexColor("#5A5F73")
IVORY = HexColor("#F6F1E8")


_UNICODE_FONT_PATHS = [
    "/Library/Fonts/Arial Unicode.ttf",
    "/System/Library/Fonts/Supplemental/Arial Unicode.ttf",
]
UNICODE_FONT = "Helvetica"
for _p in _UNICODE_FONT_PATHS:
    try:
        pdfmetrics.registerFont(TTFont("ArialUnicode", _p))
        UNICODE_FONT = "ArialUnicode"
        break
    except Exception:
        continue


def build_styles():
    base = getSampleStyleSheet()
    return {
        "title": ParagraphStyle(
            "Title", parent=base["Title"], fontName="Helvetica-Bold",
            fontSize=24, leading=28, textColor=NAVY, spaceAfter=6,
        ),
        "subtitle": ParagraphStyle(
            "Subtitle", parent=base["Normal"], fontName="Helvetica-Oblique",
            fontSize=11, leading=15, textColor=MUTED, spaceAfter=14,
        ),
        "h1": ParagraphStyle(
            "H1", parent=base["Heading1"], fontName="Helvetica-Bold",
            fontSize=17, leading=22, textColor=NAVY, spaceBefore=18, spaceAfter=8,
            keepWithNext=1,
        ),
        "h2": ParagraphStyle(
            "H2", parent=base["Heading2"], fontName="Helvetica-Bold",
            fontSize=13, leading=17, textColor=VIOLET, spaceBefore=12, spaceAfter=4,
            keepWithNext=1,
        ),
        "h3": ParagraphStyle(
            "H3", parent=base["Heading3"], fontName="Helvetica-Bold",
            fontSize=11, leading=15, textColor=NAVY, spaceBefore=9, spaceAfter=3,
            keepWithNext=1,
        ),
        "eyebrow": ParagraphStyle(
            "Eyebrow", parent=base["Normal"], fontName="Helvetica-Bold",
            fontSize=8.5, leading=12, textColor=GOLD, spaceAfter=4,
        ),
        "body": ParagraphStyle(
            "Body", parent=base["Normal"], fontName="Helvetica",
            fontSize=10.5, leading=15, textColor=colors.black, spaceAfter=6,
            alignment=TA_LEFT,
        ),
        "hint": ParagraphStyle(
            "Hint", parent=base["Normal"], fontName="Helvetica-Oblique",
            fontSize=9.5, leading=13, textColor=MUTED, spaceAfter=6,
        ),
        "pull": ParagraphStyle(
            "Pull", parent=base["Normal"], fontName="Helvetica-Oblique",
            fontSize=13, leading=18, textColor=NAVY,
            leftIndent=10, spaceBefore=4, spaceAfter=8,
        ),
        "quote": ParagraphStyle(
            "Quote", parent=base["Normal"], fontName="Helvetica-Oblique",
            fontSize=10.5, leading=15, textColor=NAVY,
            leftIndent=12, rightIndent=8, spaceBefore=4, spaceAfter=8,
        ),
        "bullet": ParagraphStyle(
            "Bullet", parent=base["Normal"], fontName="Helvetica",
            fontSize=10.5, leading=15, textColor=colors.black,
            leftIndent=14, bulletIndent=2, spaceAfter=2,
        ),
        "cta": ParagraphStyle(
            "CTA", parent=base["Normal"], fontName="Helvetica-Bold",
            fontSize=10, leading=14, textColor=GOLD, spaceAfter=8,
        ),
        "check": ParagraphStyle(
            "Check", parent=base["Normal"], fontName=UNICODE_FONT,
            fontSize=10.5, leading=16, textColor=colors.black,
            leftIndent=14, spaceAfter=2,
        ),
    }


def section_rule():
    return HRFlowable(width="100%", thickness=0.6, color=GOLD,
                      spaceBefore=10, spaceAfter=14)


def thin_rule():
    return HRFlowable(width="100%", thickness=0.3, color=HexColor("#DDDDDD"),
                      spaceBefore=4, spaceAfter=8)


def on_page(canvas, doc):
    canvas.saveState()
    canvas.setFont("Helvetica", 8)
    canvas.setFillColor(MUTED)
    canvas.drawString(20 * mm, 12 * mm, "[Markenname] · Web Copy · Entwurf V1 für Saskia")
    canvas.drawRightString(A4[0] - 20 * mm, 12 * mm, f"Seite {doc.page}")
    canvas.setStrokeColor(GOLD)
    canvas.setLineWidth(0.4)
    canvas.line(20 * mm, 15 * mm, A4[0] - 20 * mm, 15 * mm)
    canvas.restoreState()


def on_cover(canvas, doc):
    canvas.saveState()
    canvas.setFillColor(NAVY)
    canvas.rect(0, 0, A4[0], A4[1], fill=1, stroke=0)
    canvas.setFillColor(GOLD)
    canvas.rect(25 * mm, A4[1] - 50 * mm, 30 * mm, 0.8 * mm, fill=1, stroke=0)

    canvas.setFillColor(colors.white)
    canvas.setFont("Helvetica-Bold", 30)
    canvas.drawString(25 * mm, A4[1] - 68 * mm, "Web Copy · Entwurf V1")
    canvas.setFont("Helvetica", 13)
    canvas.drawString(25 * mm, A4[1] - 79 * mm, "Startseite · Über · Leistungen · Agenturen · Kontakt")

    canvas.setFillColor(HexColor("#A9B8C9"))
    canvas.setFont("Helvetica-Oblique", 11)
    canvas.drawString(25 * mm, A4[1] - 92 * mm, "Primärsprache Englisch · DE-Vorfassung zur Orientierung · 23. April 2026")

    canvas.setFillColor(colors.white)
    canvas.setFont("Helvetica", 11)
    intro = [
        "Liebe Saskia,",
        "",
        "das hier ist der erste Textentwurf für deine Website. Englisch ist die Primärsprache,",
        "Deutsch und Französisch werden erst übersetzt, wenn die englische Fassung sitzt.",
        "",
        "Du musst jetzt nichts schreiben. Nur lesen, spüren, markieren.",
        "",
        "Was ich von dir brauche:",
        "   1.  Klingt das nach dir? Wo nicht?",
        "   2.  Stimmen die fachlichen Aussagen (Qualifikationen, Sprachen, Einsatzarten)?",
        "   3.  Welche Sätze möchtest du auf keinen Fall so stehen lassen?",
        "",
        "Platzhalter [Markenname] bleibt, bis wir den Namen haben.",
        "Keine Referenzen, keine Preise, keine Zahlen werden öffentlich,",
        "bevor du sie freigegeben hast.",
    ]
    y = A4[1] - 112 * mm
    for line in intro:
        canvas.drawString(25 * mm, y, line)
        y -= 6 * mm

    canvas.setFillColor(GOLD)
    canvas.setFont("Helvetica-Bold", 11)
    canvas.drawString(25 * mm, 32 * mm, "Richtwert für Rückmeldung: diese oder nächste Woche.")
    canvas.setFillColor(HexColor("#A9B8C9"))
    canvas.setFont("Helvetica-Oblique", 9)
    canvas.drawString(25 * mm, 25 * mm,
        "Intern · nicht zur Weitergabe an Familien oder Agenturen vor Freigabe.")
    canvas.restoreState()


def P(text, style):
    return Paragraph(text, style)


def bullets(items, styles):
    return [Paragraph("•  " + t, styles["bullet"]) for t in items]


def build_story(styles):
    story = []

    # TOC
    story.append(P("Inhalt", styles["h1"]))
    toc = [
        "1.  Startseite (Home)",
        "2.  Über · Who is behind [Markenname]",
        "3.  Leistungen — drei Service-Seiten",
        "      3.1  Global Governess",
        "      3.2  Nanny Relief",
        "      3.3  Ski + Care · Zermatt",
        "4.  Für Agenturen (B2B)",
        "5.  Kontakt",
        "6.  Mikrotext (CTAs, Formulare, Footer)",
        "7.  DE-Vorfassung der Startseite",
        "8.  Offene Punkte — was von dir kommen muss",
    ]
    for t in toc:
        story.append(P(t, styles["body"]))
    story.append(section_rule())

    # --------------------------------------------------------------
    # 1 HOME
    story.append(P("1 — Startseite (Home)", styles["h1"]))

    story.append(P("HERO", styles["eyebrow"]))
    story.append(P("<i>Eyebrow:</i> [Markenname] · a premium nanny, based in Zermatt, available worldwide.", styles["body"]))
    story.append(P("Presence over performance.", styles["pull"]))
    story.append(P(
        "A state-certified educator, ski and yoga instructor, and mother of four — "
        "for families who want their children seen, not managed.",
        styles["body"]))
    story.append(P("CTA:  Request a conversation   ·   How we work", styles["cta"]))

    story.append(P("Alternative Headlines (zur Auswahl)", styles["h3"]))
    story.extend(bullets([
        "<i>A person, not a placeholder.</i>",
        "<i>Calm, even when the day is not.</i>",
        "<i>Care that holds the frame.</i>",
    ], styles))

    story.append(thin_rule())

    story.append(P("INTRO (≈ 60 Wörter)", styles["eyebrow"]))
    story.append(P(
        "[Markenname] is a premium nanny service for internationally mobile families. "
        "One qualified professional — a <i>staatlich geprüfte Erzieherin</i> "
        "(state-certified early-childhood educator), certified ski instructor, and "
        "yoga teacher — based in Zermatt, available for weekly and monthly engagements worldwide.",
        styles["body"]))
    story.append(P(
        "For children who need more than childcare: calm, attention, and a different way of being heard.",
        styles["quote"]))

    story.append(thin_rule())

    story.append(P("DREI SÄULEN (Kartenraster)", styles["eyebrow"]))

    story.append(P("Global Governess", styles["h3"]))
    story.append(P(
        "Long-form engagements, weekly or monthly, in Zermatt or wherever your family "
        "travels. One constant person for children who value continuity.",
        styles["body"]))

    story.append(P("Nanny Relief", styles["h3"]))
    story.append(P(
        "Qualified cover when your live-in nanny is on leave or unavailable. "
        "Quiet hand-over, no disruption to your household routine.",
        styles["body"]))

    story.append(P("Ski + Care · Zermatt", styles["h3"]))
    story.append(P(
        "In partnership with <b>Evolution Ski School Zermatt</b> — confident time on "
        "the slopes for the adults, patient companionship on and off the snow for the children.",
        styles["body"]))

    story.append(thin_rule())

    story.append(P("QUIET PROOF-STRIP (dezente Textzeile)", styles["eyebrow"]))
    story.append(P(
        "<i>State-certified early-childhood educator · Certified ski instructor · "
        "Yoga teacher · Mother of four · Based in Zermatt · Partner of Evolution Ski School Zermatt</i>",
        styles["quote"]))

    story.append(thin_rule())

    story.append(P("HOW WE WORK (3 Schritte)", styles["eyebrow"]))

    story.append(P("1 · Conversation", styles["h3"]))
    story.append(P(
        "Every engagement begins with a qualifying conversation. It lets us understand "
        "your family, your children, and the frame you need — and it lets you understand "
        "who you are entrusting them to.",
        styles["body"]))

    story.append(P("2 · Preparation", styles["h3"]))
    story.append(P(
        "A clear brief: routines, languages, allergies, preferences, boundaries. "
        "Written, confirmed by both sides, never assumed.",
        styles["body"]))

    story.append(P("3 · Presence", styles["h3"]))
    story.append(P(
        "Quiet, professional, present. Your children know someone is holding the frame. "
        "You know where you stand at every moment.",
        styles["body"]))

    story.append(thin_rule())

    story.append(P("CLOSING", styles["eyebrow"]))
    story.append(P("Not every family is a fit. That is the point.", styles["pull"]))
    story.append(P(
        "We work on a small number of engagements at a time. A qualifying conversation "
        "is always the first step — indicative rates from CHF, confirmed after we have spoken.",
        styles["body"]))
    story.append(P("CTA:  Request a conversation", styles["cta"]))

    story.append(PageBreak())

    # --------------------------------------------------------------
    # 2 ABOUT
    story.append(P("2 — Über · Who is behind [Markenname]", styles["h1"]))

    story.append(P("OPENING", styles["eyebrow"]))
    story.append(P("One qualified person. A rare combination.", styles["pull"]))
    story.append(P(
        "Behind [Markenname] is SG — a <i>staatlich geprüfte Erzieherin</i> "
        "(state-certified early-childhood educator), certified ski instructor, "
        "yoga teacher, and mother of four. Based in Zermatt. Available internationally.",
        styles["body"]))
    story.append(P(
        "Name und Foto erscheinen erst nach Namensentscheidung und Shooting.",
        styles["hint"]))

    story.append(thin_rule())

    story.append(P("QUALIFICATIONS", styles["eyebrow"]))
    quals = [
        "<i>Staatlich geprüfte Erzieherin</i> — state-certified early-childhood educator (Germany).",
        "<i>Fachabitur</i> — vocational higher-education entrance qualification.",
        "Studies in Social Work, with a focus on early childhood.",
        "Certified ski instructor.",
        "Certified yoga teacher.",
        "First-aid trained for working with children.",
        "Mother of four.",
    ]
    story.extend(bullets(quals, styles))
    story.append(P(
        "<i>Each qualification is documented. Certificates available on request during the qualifying conversation.</i>",
        styles["hint"]))

    story.append(thin_rule())

    story.append(P("APPROACH", styles["eyebrow"]))
    story.append(P(
        "The way a child is spoken to in the first minute of the morning sets the tone "
        "for the day. That is where the work begins.",
        styles["quote"]))
    story.append(P(
        "Children are different. Some need movement before they can sit still. Some "
        "need quiet before they can speak. Some need a clear frame more than they "
        "need a programme. That is not a diagnosis — that is attention.",
        styles["body"]))
    story.append(P(
        "This is not therapy, and it does not replace specialists. It is childcare "
        "done by a qualified educator who is used to being present without being loud.",
        styles["body"]))

    story.append(thin_rule())

    story.append(P("A NOTE ON CHILDREN WITH PARTICULAR NEEDS", styles["eyebrow"]))
    story.append(P(
        "Some children need more calm, a different pace, or a clearer frame — for "
        "reasons that may or may not carry a name. [Markenname] offers depth of "
        "attention, not labels. Specialist therapies and medical guidance remain "
        "with the professionals who hold them.",
        styles["body"]))

    story.append(PageBreak())

    # --------------------------------------------------------------
    # 3.1 Global Governess
    story.append(P("3.1 — Global Governess", styles["h1"]))
    story.append(P("FOR INTERNATIONALLY MOBILE FAMILIES", styles["eyebrow"]))
    story.append(P("One constant, wherever you travel.", styles["pull"]))
    story.append(P(
        "Weekly and monthly engagements, in Zermatt or abroad. For children who "
        "benefit from continuity and a calmer household rhythm.",
        styles["body"]))

    story.append(P("What this looks like", styles["h3"]))
    story.extend(bullets([
        "Week- or month-long engagements, single- or multi-location.",
        "One qualified educator as the constant reference for the children — not a rotating team.",
        "Clear written brief: routines, languages, school, allergies, boundaries.",
        "Discreet coordination with existing household staff (housekeepers, drivers, tutors) where present.",
    ], styles))

    story.append(P("Typical fit", styles["h3"]))
    story.extend(bullets([
        "Families based across several residences (e. g. Zurich · London · Dubai · Zermatt).",
        "Children aged roughly 3–14, single- or multilingual.",
        "Households that value a professional educator over a generalist.",
    ], styles))

    story.append(P("Indicative frame", styles["h3"]))
    story.extend(bullets([
        "Indicative rates from CHF, confirmed after the qualifying conversation.",
        "Travel, accommodation, and per diems arranged separately and transparently.",
        "Languages: English (working language), German, French.",
    ], styles))

    story.append(P("CTA:  Request a conversation", styles["cta"]))

    story.append(PageBreak())

    # --------------------------------------------------------------
    # 3.2 Nanny Relief
    story.append(P("3.2 — Nanny Relief", styles["h1"]))
    story.append(P("WHEN YOUR LIVE-IN NANNY IS UNAVAILABLE", styles["eyebrow"]))
    story.append(P("Cover that carries the household — without rewriting it.", styles["pull"]))
    story.append(P(
        "Qualified, discreet relief for established households during illness, "
        "leave, or short-term gaps.",
        styles["body"]))

    story.append(P("What this looks like", styles["h3"]))
    story.extend(bullets([
        "Engagements from a weekend to several weeks.",
        "Structured hand-over from your existing nanny or household manager — routines, allergies, preferences — read, confirmed, followed.",
        "No attempt to reinvent the household. The goal is continuity.",
        "Written close-out at the end of the engagement, ready for your returning staff.",
    ], styles))

    story.append(P("Typical fit", styles["h3"]))
    story.extend(bullets([
        "Families with an existing live-in nanny who needs leave or medical cover.",
        "Household managers or family offices coordinating on behalf of the family.",
        "Nanny agencies looking for a qualified educator to step in.",
    ], styles))

    story.append(P("Indicative frame", styles["h3"]))
    story.extend(bullets([
        "Indicative rates from CHF, confirmed after the qualifying conversation.",
        "Availability confirmed in writing before any engagement is booked.",
    ], styles))

    story.append(P("CTA:  Request a conversation", styles["cta"]))

    story.append(PageBreak())

    # --------------------------------------------------------------
    # 3.3 Ski + Care
    story.append(P("3.3 — Ski + Care · Zermatt", styles["h1"]))
    story.append(P("IN PARTNERSHIP WITH EVOLUTION SKI SCHOOL ZERMATT", styles["eyebrow"]))
    story.append(P("Your day on the slopes. Their day in steady hands.", styles["pull"]))
    story.append(P(
        "A calm companion on the mountain and back at the chalet — built for children "
        "who benefit from patience as much as technique.",
        styles["body"]))

    story.append(P("How it works", styles["h3"]))
    story.extend(bullets([
        "The ski lesson itself is delivered by <b>Evolution Ski School Zermatt</b> under its own licence and instructors.",
        "[Markenname] provides the care component: patient companionship, preparation before and transition after the lesson, quiet time at the chalet, meals, routines.",
        "One qualified educator with her own ski qualification on snow — the transition between lesson and lunch does not require a new face for the child.",
    ], styles))

    story.append(P("Typical fit", styles["h3"]))
    story.extend(bullets([
        "Families staying in Zermatt for a week or longer during the ski season.",
        "Children who find a pure group lesson too stimulating, or who benefit from a longer settling-in period.",
        "Parents who want to ski as adults without losing sight of how the children are being held.",
    ], styles))

    story.append(P("Partner note", styles["h3"]))
    story.append(P(
        "[Markenname] does not operate as a ski school. All instruction in Zermatt "
        "is delivered by <b>Evolution Ski School Zermatt</b> (Fabrizio Pavan). The care "
        "element is provided by [Markenname].",
        styles["quote"]))

    story.append(P("CTA:  Request a conversation", styles["cta"]))

    story.append(PageBreak())

    # --------------------------------------------------------------
    # 4 B2B
    story.append(P("4 — Für Agenturen (B2B)", styles["h1"]))
    story.append(P("PLACEMENTS VIA YOUR AGENCY", styles["eyebrow"]))
    story.append(P("A qualified educator, placed through channels you already trust.", styles["pull"]))
    story.append(P(
        "[Markenname] is available for placements via selected nanny and governess "
        "agencies. All contractual, logistical, and communication frames sit with "
        "your agency. We focus on the child.",
        styles["body"]))

    story.append(P("What we bring", styles["h3"]))
    story.extend(bullets([
        "<i>Staatlich geprüfte Erzieherin</i> (state-certified early-childhood educator) with additional ski and yoga qualifications.",
        "Depth of attention for children with particular needs, without diagnostic labelling.",
        "Clean hand-overs, written close-outs, discreet communication.",
        "Working languages: English, German, French.",
    ], styles))

    story.append(P("How we work with agencies", styles["h3"]))
    story.extend(bullets([
        "Single placements, relief cover, or recurring ski-season engagements.",
        "Full transparency on availability, qualifications, and indicative rates.",
        "Reciprocal visibility where appropriate (logo placement on both sides, subject to written agreement).",
    ], styles))

    story.append(P("CTA:  Request a placement conversation", styles["cta"]))

    # --------------------------------------------------------------
    # 5 Contact
    story.append(P("5 — Kontakt", styles["h1"]))
    story.append(P("Every engagement begins with a conversation.", styles["pull"]))
    story.append(P(
        "The qualifying conversation lets us understand your family and lets you "
        "understand who you are entrusting your children to. Indicative rates from CHF "
        "are shared afterwards, in writing. We take on a small number of engagements at a time.",
        styles["body"]))

    story.append(P("Formularfelder", styles["h3"]))
    story.extend(bullets([
        "Name",
        "Email",
        "Phone (optional)",
        "Country / current base",
        "Approximate period of engagement",
        "Number and age of children",
        "Anything specific we should know before the conversation (free text)",
        "Consent checkbox: „I agree to the processing of my data for the purpose of this enquiry.“",
    ], styles))

    story.append(P("Alternative Kanäle", styles["h3"]))
    story.extend(bullets([
        "Email: hello@[markenname].xx  (nach Domain-Entscheidung)",
        "WhatsApp Business: folgt",
        "Phone: separate Business-Nummer, folgt",
    ], styles))

    story.append(PageBreak())

    # --------------------------------------------------------------
    # 6 Microcopy
    story.append(P("6 — Mikrotext", styles["h1"]))

    story.append(P("CTAs", styles["h3"]))
    story.extend(bullets([
        "Primary:  <b>Request a conversation</b>",
        "Alternative (B2B):  <b>Request a placement conversation</b>",
        "Soft:  <i>Read more →</i>  ·  <i>How we work</i>",
    ], styles))

    story.append(P("Form states", styles["h3"]))
    story.extend(bullets([
        "Success:  <i>Thank you. Expect a reply within two working days.</i>",
        "Error:  <i>Something did not send. Please try again, or write to hello@[markenname].xx.</i>",
        "Validation:  <i>Please enter a valid email address.</i>",
    ], styles))

    story.append(P("Footer claim", styles["h3"]))
    story.append(P(
        "[Markenname] — a premium nanny, based in Zermatt, available worldwide.",
        styles["quote"]))

    story.append(P("Cookie / consent banner (Kurzfassung)", styles["h3"]))
    story.append(P(
        "We use essential cookies to make this site work and optional analytics cookies "
        "to understand how it is used. You can accept or decline.  [Accept] [Decline] [Preferences]",
        styles["body"]))

    # --------------------------------------------------------------
    # 7 DE
    story.append(P("7 — DE-Vorfassung der Startseite", styles["h1"]))
    story.append(P(
        "Nur zur Orientierung, damit sich der Ton auf Deutsch spürbar macht. "
        "Die finale DE-Fassung entsteht nach EN-Freigabe via DeepL und muttersprachlichem Review.",
        styles["hint"]))

    story.append(P("HERO (DE)", styles["eyebrow"]))
    story.append(P("Präsenz statt Programm.", styles["pull"]))
    story.append(P(
        "Staatlich geprüfte Erzieherin, zertifizierte Skilehrerin, Yogalehrerin, "
        "Mutter von vier — für Familien, die ihre Kinder gesehen wissen wollen, nicht verwaltet.",
        styles["body"]))
    story.append(P("CTA:  Gespräch anfragen   ·   So arbeiten wir", styles["cta"]))

    story.append(P("INTRO (DE, ≈ 60 Wörter)", styles["eyebrow"]))
    story.append(P(
        "[Markenname] ist eine Premium-Nanny für international mobile Familien. "
        "Eine qualifizierte Bezugsperson — staatlich geprüfte Erzieherin, zertifizierte "
        "Skilehrerin, Yogalehrerin — mit Basis in Zermatt, einsatzfähig für Wochen- und "
        "Monatsengagements weltweit.",
        styles["body"]))
    story.append(P(
        "Für Kinder, die mehr brauchen als Betreuung: Ruhe, Aufmerksamkeit und eine "
        "andere Art, gehört zu werden.",
        styles["quote"]))

    story.append(P("DREI SÄULEN (DE)", styles["eyebrow"]))
    story.append(P("Global Governess", styles["h3"]))
    story.append(P(
        "Wochen- und Monatseinsätze, in Zermatt oder dort, wohin Ihre Familie reist. "
        "Eine konstante Bezugsperson für Kinder, die Kontinuität brauchen.",
        styles["body"]))

    story.append(P("Nanny-Vertretung", styles["h3"]))
    story.append(P(
        "Qualifizierte Vertretung, wenn Ihre Live-in-Nanny ausfällt oder Urlaub braucht. "
        "Ruhige Übergabe, keine Störung im Haushaltsrhythmus.",
        styles["body"]))

    story.append(P("Ski + Care · Zermatt", styles["h3"]))
    story.append(P(
        "In Kooperation mit der <b>Evolution Ski School Zermatt</b> — Skitage für die "
        "Erwachsenen, geduldige Begleitung für die Kinder auf und neben der Piste.",
        styles["body"]))

    story.append(PageBreak())

    # --------------------------------------------------------------
    # 8 Offene Punkte
    story.append(P("8 — Offene Punkte (bitte prüfen)", styles["h1"]))
    story.append(P(
        "Diese Punkte brauchen deinen Input, bevor wir V2 machen und DE/FR übersetzen lassen.",
        styles["hint"]))

    checks = [
        "<b>Markenname</b> — sobald entschieden, globale Ersetzung.",
        "<b>„Mother of four“</b> — Formulierung so ok? Möchtest du das überhaupt auf der Website haben?",
        "<b>Fabrizio Pavan / Evolution Ski School</b> — Text-Freigabe für Partner-Nennung auf Website einholen.",
        "<b>Indicative rate floor</b> — ab welchem CHF-Richtpreis sollen wir schreiben? (Fehlt im Entwurf bewusst, bis wir es festlegen.)",
        "<b>Sprachen</b> — „working language English“ — stimmt das? Oder Deutsch primär in CH?",
        "<b>Anonymisierter Case (Lotta)</b> — dürfen wir eine kurze Textfassung vorbereiten und mit Felix abstimmen?",
        "<b>Stil-Check</b> — welche Passagen klingen nicht nach dir? Bitte markieren oder ankreuzen.",
        "<b>Fehlende Themen</b> — gibt es etwas, das im Entwurf fehlt und unbedingt drin sein muss?",
    ]
    for c in checks:
        story.append(P("☐  " + c, styles["check"]))

    story.append(Spacer(1, 6 * mm))
    story.append(P(
        "Wenn du fertig bist: markiert-PDF zurück per Mail, oder Anmerkungen in "
        "einer kurzen Sprachnachricht. Beides geht.",
        styles["body"]))

    return story


def main() -> None:
    styles = build_styles()
    doc = BaseDocTemplate(
        str(OUTPUT), pagesize=A4,
        leftMargin=20 * mm, rightMargin=20 * mm,
        topMargin=20 * mm, bottomMargin=22 * mm,
        title="Web Copy V1 · [Markenname]",
        author="SG_Content_Projekt",
    )
    content_frame = Frame(
        doc.leftMargin, doc.bottomMargin,
        doc.width, doc.height, id="content"
    )
    cover_frame = Frame(0, 0, A4[0], A4[1], id="cover",
                        leftPadding=0, rightPadding=0, topPadding=0, bottomPadding=0)
    doc.addPageTemplates([
        PageTemplate(id="cover", frames=[cover_frame], onPage=on_cover),
        PageTemplate(id="content", frames=[content_frame], onPage=on_page),
    ])

    story = [NextPageTemplate("content"), PageBreak()]
    story.extend(build_story(styles))
    doc.build(story)
    print(f"wrote {OUTPUT}")


if __name__ == "__main__":
    main()
